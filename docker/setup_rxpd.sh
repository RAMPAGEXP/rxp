...
GENESIS_FILE="$HOME"/.rxp/config/genesis.json
if [ -f "$GENESIS_FILE" ]; then
  echo "$GENESIS_FILE exists..."
else
  echo "$GENESIS_FILE does not exist. Generating..."

  rxpd init --chain-id "$CHAIN_ID" "$MONIKER"
  ...
  sed -i 's/keyring-backend = "os"/keyring-backend = "test"/' "$HOME"/.rxp/config/client.toml
fi

APP_TOML_CONFIG="$HOME"/.rxp/config/app.toml
APP_TOML_CONFIG_NEW="$HOME"/.rxp/config/app_new.toml
CONFIG_TOML_CONFIG="$HOME"/.rxp/config/config.toml
...
if ! rxpd keys show validator $KEYRING; then
  (echo "$PASSWORD"; echo "$PASSWORD") | rxpd keys add validator $KEYRING

  # hardcode the validator account for this instance
  echo "$PASSWORD" | rxpd add-genesis-account validator "1000000000$STAKE,1000000000$FEE" $KEYRING

  # (optionally) add a few more genesis accounts
  for addr in "$@"; do
    echo $addr
    rxpd add-genesis-account "$addr" "1000000000$STAKE,1000000000$FEE"
  done

  # submit a genesis validator tx
  ## Workraround for https://github.com/cosmos/cosmos-sdk/issues/8251
  (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | rxpd gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID" --amount="250000000$STAKE" $KEYRING
  ## should be:
  # (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | rxpd gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID"
  rxpd collect-gentxs
fi
